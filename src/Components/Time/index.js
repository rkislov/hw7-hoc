...
const computedAfterDate = (date) => {
  ...
  if (diffDays > 0) {
    return { date: `${diffDays} дней назад` };
  }
  if (diffHours > 0) {
    return { date: `${diffDays} часов назад` };
  }
  if (diffMinutes > 0) {
    return { date: `${diffMinutes} минут назад` };
  }
  if (diffSeconds > 0) {
    return { date: `${diffSeconds} секунд назад` };
  }
...
const handlerComponent = (Component) => {
  return function (props) {
    return <Component {...computedAfterDate(props.date)} />;
  };
};
